  ...
  function reflectInjectables(targetClass: Type): ClassInjectable[] {
    ...
    return Object.keys(classProperties).map((propertyKey) => {
      ...
      if (!propertyMetadataItems && !propertyParamType) {
        throw new UndefinedDependencyError(`Automock encountered an error while attempting to detect a token or type for the dependency for property '${propertyKey}' in the class '${targetClass.name}'.
