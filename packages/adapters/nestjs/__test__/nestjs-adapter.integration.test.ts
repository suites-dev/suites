...
describe('NestJS Automock Adapter Integration Test', () => {
  ...
  describe('reflecting classes with undefined constructor dependencies', () => {
    it.each([
      [
        ClassWithUndefinedDependency,
        "'ClassWithUndefinedDependency' is missing a token for the dependency at index [0], did you forget to inject it using @Inject()?",
      ],
      [
        ClassWithUndefinedDependencyProps,
        "Automock has failed to reflect 'ClassWithUndefinedDependencyProps.dependency' property, did you forget to inject it using @Inject()?",
      ],
    ])(
      'should fail with an error indicating that the dependency is not defined',
      (type: Type, error: string) => {
        expect(() => reflectorFactory.reflectDependencies(type)).toThrowError(error);
      }
    );
  });
});
