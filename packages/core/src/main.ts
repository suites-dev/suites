...
import { AdapterNotFoundError } from './types';
...
function createTestbedBuilder<TClass>(
  ...
  try {
    ...
    return UnitBuilder.create<TClass>(mockFn, unitMocker, adapter);
  } catch (error: unknown) {
    throw new AdapterNotFoundError(
      `Automock requires an adapter to seamlessly integrate with different dependency injection frameworks.
