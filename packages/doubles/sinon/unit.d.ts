...
import type { Mocked as SinonMocked, Stub as SinonStub } from '.';
import type { ArgsType } from '@suites/types.doubles';

/**
 * Sinon Adapter Type Augmentation
 *
 * This file bridges the abstract types from @suites/unit to Sinon-specific concrete implementations
 * through TypeScript module augmentation. When this adapter is installed, these augmentations
 * override the base abstract types with Sinon's stubbing framework capabilities.
 *
...
declare module '@suites/unit' {
  ...
  /**
   * Sinon-specific mocked instance type where all methods are Sinon stubs.
   *
   * This augmentation overrides the base `Mocked<T>` type from `@suites/unit`
   * with Sinon's concrete stub implementation when the Sinon adapter is installed.
   *
...
declare module '@suites/core.unit' {
   ...
   * - All retrieved mocks are fully typed with Sinon's stubbing capabilities
   * - Use this to configure stub behavior and assertions after TestBed compilation
   *
   * @example
   * ```ts
   ...
   *
   * // Configure stub behaviors
   * userRepo.findById.resolves(testUser);
  ...
  export interface MockOverride<TDependency, TClass> {
     ...
     *
     * Use this when you need to change stub behavior after TestBed compilation.
     * The `stubFn` parameter provides Sinon stub functions that you can configure,
     ...
     *   .mock(UserRepository)
     *   .impl(stubFn => ({
     *     findById: stubFn().resolves(testUser),
     *     save: stubFn().resolves(void 0)
     *   }))
     *   .compile();
     *
     * // Later in test, modify behavior
     * const repo = unitRef.get(UserRepository);
     * repo.findById.resolves(differentUser);
     * ```
     *
     * @since 3.0.0
     */
